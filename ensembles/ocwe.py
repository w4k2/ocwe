...
class OCWE(ClassifierMixin, BaseEnsemble):
    ...
    def partial_fit(self, X, y, classes=None):
            ...
            for i in range(n_clusters):
                if len(samples[i]) != 0:
                    self.ensemble_min.append(
                        clone(OneClassSVM(nu=0.1, kernel="rbf")).fit(samples[i]))
        ...
        # ________________________________________
        # Combine all weights
        self.weights_min = self.alpha * distances_min + self.beta * \
            scores_min + self.gamma * age_min + self.delta * irw_min
        self.weights_maj = self.alpha * distances_maj + self.beta * \
