...
def main():
    streams = []
    streams.append(("streams/real/covtypeNorm-1-2vsAll-pruned.arff",                 2000,  10))
    streams.append(("streams/real/poker-lsn-1-2vsAll-pruned.arff",                   2000,  10))
    ...
    for stream_name, chunk_size, n_chunks in streams:
        ...
        # # -  Print best params
        # # -----------------------
        # metrics_array1 = load_data(stream_name, metric_names, steps, "Alpha", "Beta")
        # argmax1 = np.where(metrics_array1[0, :, :] == metrics_array1[0, :, :].max())
        # metrics_array2 = load_data(stream_name, metric_names, steps, "Gamma", "Delta")
        # argmax2 = np.where(metrics_array2[0, :, :] == metrics_array2[0, :, :].max())
        # print("alpha=%0.1f," % (argmax1[1][0]/10), "beta=%0.1f," % (argmax1[0][0]/10), "gamma=%0.1f," % (argmax2[1][0]/10), "delta=%0.1f" % (argmax2[0][0]/10))
...
def save_grid_results(param_1, param_2, metrics_array, metric_names, stream_name):
    for idx, metric in enumerate(metric_names):
        ...
        filepath = "results/param_setup_real_grid/vals/%s" % filename

        if not os.path.exists("results/param_setup_real_grid/vals"):
            os.makedirs("results/param_setup_real_grid/vals")
...
def plot_grid_results(param_1, param_2, metrics_array, metric_names, stream_name, resolution):
    for idx, metric in enumerate(metric_names):
        ...
        if not os.path.exists("results/param_setup_real_grid/plots"):
            os.makedirs("results/param_setup_real_grid/plots")

        filename = ("%s%s_" % (param_1[0], param_2[0])) + \
            metric + "__" + (stream_name.split("/")[-1])[0:-5]

        plt.tight_layout()
        plt.savefig("results/param_setup_real_grid/plots/%s.png" % filename)
        plt.savefig("results/param_setup_real_grid/plots/%s.eps" % filename)
